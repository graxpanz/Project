<div class="card shadow">
    ...
    <form action="/service/update" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-12 px-1 px-md-5">
                    <div class="form-group text-center">
                        ...
                        <div class="position-relative mb-3 mx-auto" style="width: 360px;">
                            <img id="preview"
                                src="<?= !empty($service['image']) ? '/assets/uploads/service/' . $service['image'] : '/assets/images/no-image.jpg' ?>"
                                alt="Service Preview"
                                class="img-thumbnail"
            ...
            <div class="row">
                <div class="col col-md-6 px-1 px-md-5 mt-3">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="name" id="name" placeholder="ชื่อบริการ" value="<?= $service['name'] ?>" required>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control" name="service_type_id" id="service_type_id" required>
                            ...
                            <?php foreach ($service_types as $service_type): ?>
                                <option value="<?= $service_type['service_type_id'] ?>" <?= $service_type['service_type_id'] == $service['service_type_id'] ? 'selected' : '' ?>>
                                    <?= $service_type['name'] ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="description" id="description" rows="6" placeholder="รายละเอียด"><?= $service['description'] ?></textarea>
                    </div>
                </div>
                <div class="col col-md-6 px-1 px-md-5 mt-3">
                    <div class="form-group">
                        <label for="price">ราคา (บาท)</label>
                        <input type="number" class="form-control" name="price" id="price" placeholder="ราคา" min="0.00" step="1.00" value="<?= $service['price'] ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="time">เวลาในการบริการ (นาที)</label>
                        <input type="number" class="form-control" name="time" id="time" placeholder="เวลา" min="0" step="1" value="<?= $service['time'] ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="is_active">สถานะการใช้งาน</label>
                        <select class="form-control" name="is_active" id="is_active" required>
                            <option value="1" <?= $service['is_active'] == 1 ? 'selected' : '' ?>>เปิดใช้งาน</option>
                            <option value="0" <?= $service['is_active'] == 0 ? 'selected' : '' ?>>ปิดใช้งาน</option>
                        </select>
                    </div>
                </div>
            </div>
