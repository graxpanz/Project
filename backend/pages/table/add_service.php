...
<head>
    ...
    <title>หน้าเพิ่มข้อข้อมูลการบริการ</title>
                                    ...
                                    <form action="insert_service.php" method="POST">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control" id="service_type_id" name="service_type_id"
                                                required>
                                                <?php
                                                // Using PDO to fetch service types
                                                $sql = "SELECT * FROM service_type ORDER BY service_type_name";
                                                $stmt = $conn->query($sql);
                                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                                    echo "<option value='" . htmlspecialchars($row['service_type_id']) . "'>" . htmlspecialchars($row['service_type_name']) . "</option>";
                                                }
                                                ?>
                                            </select>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="number" class="form-control" id="service_time"
                                                name="service_time" required>
                                            <div id="service_time_help" class="form-text">
                                                กรุณากรอกจำนวนเวลาในการให้บริการเป็นนาทีเท่านั้น</div>
                                        </div>
                                        <div class="mb-3">
                                            <label for="service_detail" class="form-label">รายละเอียด</label>
                                            <input type="text" class="form-control" id="service_detail"
                                                name="service_detail" required>
                                        </div>
